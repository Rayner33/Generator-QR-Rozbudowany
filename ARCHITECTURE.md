...
- **GeoLite2 Microservice Security:** System analityczny posiada dedykowany, wydzielony serwer Node.js (Proxy) obsługujący bazę MaxMind GeoLite2. Został on obudowany dwuwarstwowym systemem zabezpieczeń (restrykcje CORS po stronie serwera tylko dla zaufanych domen oraz ukryty klucz API wysyłany w nagłówkach HTTP) w celu ochrony bazy oraz serwera przed nieautoryzowanymi pingami (automated bots) oraz kradzieżą zasobów (hotlinking). Ponadto serwis posiada system inteligentnego "Graceful Fallback", zastępujący techniczne błędy (np. 404/400 z Nginx) statusem 200 z anonimowymi wartościami dla bezpieczeństwa czystości konsoli przeglądarki użytkownika.
